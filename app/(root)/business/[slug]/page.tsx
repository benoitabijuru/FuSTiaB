...
import { getBusinessPostBySlug } from "@/lib/actions/businessPost.action";
...
openGraph: {
images: [businessPost.imageUrl],
},
}
}
  const businessPostDetails = async ( { params: { slug }}: SearchParamsProps) => {
    const businessPost = await getBusinessPostBySlug(slug);

    return (
      <section className="justify-center wrapper">
      <div className="">
          <p className="text-[14px] py-3">Business | <span className="text-orange-700">{businessPost.category.name}</span></p>
        
        <div className="">
          <h1 className=" h1-bold ">{businessPost.title}</h1>
        </div>
        <div className="">
          <p className="py-10 text-3xl italic">{businessPost.description}</p>
        </div>
        <hr />
        <div className="py-5 ">
      ...
      <div dangerouslySetInnerHTML={{ __html: businessPost.content }} className="text-2xl"/>
      </div>
